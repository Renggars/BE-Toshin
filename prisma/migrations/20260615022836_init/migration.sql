...
CREATE TABLE `master_masalah_andon` (
    ...
    UNIQUE INDEX `master_masalah_andon_nama_masalah_kategori_key`(`nama_masalah`, `kategori`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- CreateTable
CREATE TABLE `kategori_mesin` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `nama` VARCHAR(191) NOT NULL,

    UNIQUE INDEX `kategori_mesin_nama_key`(`nama`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `master_line` (
    ...
    UNIQUE INDEX `master_line_nama_key`(`nama`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `master_kategori_dokumen` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `nama` VARCHAR(191) NOT NULL,

    UNIQUE INDEX `master_kategori_dokumen_nama_key`(`nama`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `notification` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `fk_id_user` INTEGER NOT NULL,
    `tipe` ENUM('RPH_ASSIGNED', 'ANDON_CALL', 'ANDON_RESOLVED', 'ANNOUNCEMENT') NOT NULL,
...
CREATE TABLE `document` (
    ...
    `fk_uploaded_by` INTEGER NOT NULL,
    `fk_id_mesin` INTEGER NULL,
    `fk_id_produk` INTEGER NULL,
...
CREATE TABLE `user` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `role` ENUM('OPERATOR', 'QUALITY', 'MAINTENANCE', 'DIE_MAINT', 'ENGINEERING', 'MARKETING', 'COMMERCIAL', 'PPIC', 'HCPGA', 'WRH_CIBITUNG', 'GA', 'WAREHOUSE', 'PURCHASING', 'HC', 'ACCOUNTING', 'FINANCE', 'SUPERVISOR', 'MANDOR', 'ADMIN', 'HR') NOT NULL DEFAULT 'OPERATOR',
    ...
    `line` VARCHAR(191) NOT NULL,
    `tipeOperator` ENUM('OPERATOR_LAPANGAN', 'OPERATOR_OFFICE') NULL,
    `no_reg` VARCHAR(191) NOT NULL,

    UNIQUE INDEX `user_uid_nfc_key`(`uid_nfc`),
    UNIQUE INDEX `user_no_reg_key`(`no_reg`),
...
-- CreateTable
CREATE TABLE `mandor_task` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `fk_id_supervisor` INTEGER NOT NULL,
    `fk_id_mandor` INTEGER NOT NULL,
    `judul` VARCHAR(191) NOT NULL,
    ...
    INDEX `mandor_task_fk_id_mandor_idx`(`fk_id_mandor`),
    INDEX `mandor_task_fk_id_supervisor_idx`(`fk_id_supervisor`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `operator_announcement` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `fk_id_mandor` INTEGER NOT NULL,
    `fk_id_operator` INTEGER NOT NULL,
    `pesan` TEXT NOT NULL,
    `is_read` BOOLEAN NOT NULL DEFAULT false,
    `broadcast_id` VARCHAR(191) NULL,
    `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),

    INDEX `operator_announcement_fk_id_mandor_idx`(`fk_id_mandor`),
    INDEX `operator_announcement_fk_id_operator_idx`(`fk_id_operator`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `mesin` ADD CONSTRAINT `mesin_fk_id_kategori_fkey` FOREIGN KEY (`fk_id_kategori`) REFERENCES `kategori_mesin`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `document` ADD CONSTRAINT `document_fk_uploaded_by_fkey` FOREIGN KEY (`fk_uploaded_by`) REFERENCES `user`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `document` ADD CONSTRAINT `document_fk_id_mesin_fkey` FOREIGN KEY (`fk_id_mesin`) REFERENCES `mesin`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `mandor_task` ADD CONSTRAINT `mandor_task_fk_id_supervisor_fkey` FOREIGN KEY (`fk_id_supervisor`) REFERENCES `user`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `mandor_task` ADD CONSTRAINT `mandor_task_fk_id_mandor_fkey` FOREIGN KEY (`fk_id_mandor`) REFERENCES `user`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `operator_announcement` ADD CONSTRAINT `operator_announcement_fk_id_mandor_fkey` FOREIGN KEY (`fk_id_mandor`) REFERENCES `user`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `operator_announcement` ADD CONSTRAINT `operator_announcement_fk_id_operator_fkey` FOREIGN KEY (`fk_id_operator`) REFERENCES `user`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
